...
def generate_dataset():
    ...
    EXPECTED_COLUMNS = ['timestamp', 'src_ip', 'target_ip', 'attack_label', 'attack_type', 'packets_per_second', 'mqtt_publish_rate', 'broker_response_latency_ms', 'device_heap_free_bytes', 'auth_attempt_rate', 'auth_failure_rate', 'unique_passwords_tried', 'result_code', 'password_length', 'payload_entropy', 'auth_success_rate', 'credential_entropy', 'duplicate_payload_rate', 'msg_timestamp_delta_ms', 'motion', 'arm', 'inter_arrival_mean_ms', 'inter_arrival_std_ms', 'consecutive_failures', 'session_attempt_count', 'session_failure_rate', 'latency_zscore']
    ...
    for f in target_files:
        try:
            ...
            if list(df.columns) == EXPECTED_COLUMNS:
                # Filter out any extra header rows written during appends
                df = df[df['timestamp'] != 'timestamp']
                dfs.append(df)
                print(f"  + Loaded: {os.path.basename(f)} ({len(df)} rows, 27 cols)")
            else:
                print(f"  ⚠️ Warning: {os.path.basename(f)} ignored! Column schema does not match.")
