...
app.post('/api/attack/start', (req, res) => {
  ...
  try {
    ...
    console.warn(`[PCAP WARNING] Could not write dummy PCAP file: ${err.message}`);
  }

  const tcpdumpProcess = spawn('tcpdump', ['-i', 'any', 'port', '1883', '-w', pcapPath]);
  tcpdumpProcess.on('error', (err) => console.warn(`[PCAP WARNING] Failed to start tcpdump: ${err.message}`));
  activeProcesses[`${type}_pcap`] = tcpdumpProcess;

  attackProcess.on('error', (err) => console.error(`[${type}] SPAWN ERROR: ${err.message}`));
