...
title_md = """# Smart Home Threat Simulation Platform
## Intrusion Detection System (ML-IDS) - Random Forest Classifier

This notebook trains a Random Forest model using the **full 27-column dataset (25 ML features)** generated from our local IoT threat simulations. It is designed to maximize detection accuracy across `BRUTE_FORCE`, `DOS`, `REPLAY`, and `NORMAL` traffic."""
...
We drop the `attack_type` to prevent target leakage, leaving us with **25 features** for training. Non-numeric columns (src_ip, target_ip, timestamp) are Label-Encoded so the Random Forest can extract context from them (e.g. attacker IP patterns)."""
