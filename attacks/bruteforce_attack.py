...
# ─────────────────────────────────────────────────────────────
# ML FEATURE SCHEMA — 27 columns (26 features + 1 label)
# ─────────────────────────────────────────────────────────────
ML_HEADERS = [
    # ── Identity ──────────────────────────────────────────────
    ...
    "session_failure_rate", # 26 Cumulative failures / total attempts
...
class MLFeatureTracker:
    """
    Thread-safe rolling window tracker for computing all 26 ML features.
...
class MLAttemptLogger:
    """
    Logs all 26 ML features directly to dataset/logs/brute_attempts_*.csv
    ...
    def __init__(self, target_ip, src_ip, tracker: MLFeatureTracker):
        ...
        self.sniffer.connect(self.target_ip, 1883, 60)
    ...
    def log_attempt(self, username, password, rc, latency_ms, result_str, event_time):
        """Compute all 26 ML features and log to both CSV and JSON in dataset/logs/."""
        ...
        # ── Build unified 27-column record (matches combined_ml_dataset schema) ──
...
def main():
    ...
    print(f"   → Both files contain all 27 columns")
