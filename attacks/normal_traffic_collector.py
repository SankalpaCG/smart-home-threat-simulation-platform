...
Logs all 26 ML features with attack_label=0 (NORMAL).
...
class NormalTrafficCollector:
    def __init__(self, broker, port, username, password, duration, phase, fast=False, count_limit=0):
        self.broker   = broker
        self.port     = port
        ...
        # Start background sniffer to capture REAL physical events
        self.sniffer = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1, f"sniffer_{random.getrandbits(12)}")
        self.sniffer.username_pw_set(self.username, self.password)
    ...
    def _run_single(self, start_time):
        ...
        with self.lock:
            ...
            if self.count % 10 == 0:
                ...
                status = "✅" if rc == 0 else "❌"
                sys.stdout.write(f"\r{status} Record {self.count:4d} | Latency: {latency:6.1f}ms | Elapsed: {int(elapsed)}s ")
                sys.stdout.flush()
    ...
    def run(self):
        ...
        if self.fast:
            ...
            limit = self.count_limit if self.count_limit > 0 else 100000
            with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
                futures = []
                for _ in range(limit):
                    futures.append(executor.submit(self._run_single, start_time))
                concurrent.futures.wait(futures)
        else:
            while time.time() - start_time < self.duration:
                if self.count_limit > 0 and self.count >= self.count_limit:
                    break
                ...
                status = "✅" if rc == 0 else "❌"
                sys.stdout.write(
                    f"\r{status} Record {self.count:4d} | "
                ...
                if time.time() - start_time + sleep_time > self.duration and self.count_limit == 0:
                    break
                time.sleep(sleep_time)

        print(f"\n\n✅ Collection complete!")
        print(f"   Records collected : {self.count}")
        print(f"   Successful conns  : {self.success}/{self.count}")
...
def main():
    ...
    parser.add_argument("--broker",   default="192.168.20.237")
    ...
    parser.add_argument("--fast",     action="store_true",    help="Generate data with zero delay")
    parser.add_argument("--count",    type=int, default=0,    help="Stop after N records")
    args = parser.parse_args()

    collector = NormalTrafficCollector(
        args.broker, args.port, args.username,
        args.password, args.duration, args.phase,
        args.fast, args.count
    )
    collector.run()
